...
alias doom="cd /Users/vegard/dev/personal/terminal-doom && zig-out/bin/terminal-doom"
...
alias nvim-lazy="NVIM_APPNAME=lazyvim-nvim nvim"
alias nvim-kick="NVIM_APPNAME=kickstart-nvim nvim"

function nvims() {
  items=("default" "kickstart-nvim" "lazyvim-nvim" "vanilla")
...
alias container_example="container run -it --name=\"bekk-123\" --volume $(pwd):$(pwd) base-opencode"
...
# usage: worktree [name]
# worktree          - fzf pick from all worktrees
# worktree <name>   - create worktree
worktree-old() {
  local name="$1"
  if [[ -n "$name" ]]; then
  ...
  # no args: pick from all worktrees with fzf
  local selection
